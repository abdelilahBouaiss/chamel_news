...
class CC_Love_Count {
	...
	function ajax( $post_id ) {
		...
		if ( isset( $_POST['post_id'] ) ) {
			echo esc_html( $this->love( intval($_POST['post_id']), 'update' ) );
		}
		else {
			echo esc_html( $this->love( intval($_POST['post_id']), 'get' ) );
		}
	...
	function get() {
		...
		return '<span class="project-like"> <a href="#" class="cc-love '.$class .'" data-id="'. $post->ID .'"><span class="updated-counter">'. $output .'</span> <i class="fa fa-heart"></i></a></span>';	
	}
